<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
    <div class="app-brand demo">
        <a href="{{ route('dashboard') }}" class="app-brand-link">
            ...
            <span class="app-brand-text demo menu-text fw-semibold ms-2">{{ __('app.name') }}</span>
        </a>
    ...
    <ul class="menu-inner py-1">
        @auth
            @if(auth()->user()->hasRole('super_admin'))
                @include('layouts.partials.menus.super-admin')
            @elseif(auth()->user()->hasRole('hospital_admin'))
                @include('layouts.partials.menus.hospital-admin')
            @elseif(auth()->user()->hasRole('doctor'))
                @include('layouts.partials.menus.doctor')
            @elseif(auth()->user()->hasRole('pharmacist'))
                @include('layouts.partials.menus.pharmacist')
