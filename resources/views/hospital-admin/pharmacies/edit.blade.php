...
@section('title', __('pharmacies.pharmacy') . ' — ' . $pharmacy->name)
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex align-items-center">
                ...
                <h5 class="mb-0">{{ __('app.edit') }}: {{ $pharmacy->name }}</h5>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('hospital-admin.pharmacies.update', $pharmacy) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="country_id" class="form-select @error('country_id') is-invalid @enderror" required>
                                ...
                                @foreach($countries as $country)
                                    <option value="{{ $country->id }}" @selected(old('country_id', $pharmacy->country_id) == $country->id)>{{ $country->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="city_id" class="form-select @error('city_id') is-invalid @enderror" required>
                                ...
                                @foreach($cities as $city)
                                    <option value="{{ $city->id }}" @selected(old('city_id', $pharmacy->city_id) == $city->id)>{{ $city->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="address" rows="2" class="form-control @error('address') is-invalid @enderror" required>{{ old('address', $pharmacy->address) }}</textarea>
                            @error('address')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">{{ __('pharmacies.type') }} <span class="text-danger">*</span></label>
                            <select name="type" class="form-select @error('type') is-invalid @enderror" required>
                                <option value="in_hospital" @selected(old('type', $pharmacy->type?->value) === 'in_hospital')>{{ __('pharmacies.pharmacy_type_in_hospital') }}</option>
                                <option value="external" @selected(old('type', $pharmacy->type?->value) === 'external')>{{ __('pharmacies.pharmacy_type_external') }}</option>
                                <option value="chain" @selected(old('type', $pharmacy->type?->value) === 'chain')>{{ __('pharmacies.pharmacy_type_chain') }}</option>
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                <option value="active" @selected(old('status', $pharmacy->status?->value) === 'active')>{{ __('pharmacies.status_active') }}</option>
                                <option value="inactive" @selected(old('status', $pharmacy->status?->value) === 'inactive')>{{ __('pharmacies.status_inactive') }}</option>
                                <option value="suspended" @selected(old('status', $pharmacy->status?->value) === 'suspended')>{{ __('pharmacies.status_suspended') }}</option>
                            </select>
                        ...
                        <div class="col-md-4 d-flex align-items-end">
                            <div class="form-check mt-2">
                                <input class="form-check-input" type="checkbox" name="is_24_hours" id="is_24_hours" value="1"
                                    @checked(old('is_24_hours', $pharmacy->is_24_hours))>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="time" name="opening_time" class="form-control @error('opening_time') is-invalid @enderror"
                                value="{{ old('opening_time', $pharmacy->opening_time) }}">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="time" name="closing_time" class="form-control @error('closing_time') is-invalid @enderror"
                                value="{{ old('closing_time', $pharmacy->closing_time) }}">
                        ...
                        <div class="col-md-6">
                            ...
                            @if($pharmacy->logo)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $pharmacy->logo) }}" alt="{{ $pharmacy->name }}" height="60" class="rounded">
                                </div>
                            @endif
