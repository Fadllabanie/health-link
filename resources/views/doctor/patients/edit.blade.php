...
@section('title', __('patients.edit_patient'))
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex align-items-center">
                <a href="{{ route('doctor.patients.show', $patient) }}" class="btn btn-sm btn-outline-secondary me-3">
                    <i class="bx bx-arrow-back"></i>
                </a>
                <h5 class="mb-0">{{ __('patients.edit_patient') }}</h5>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('doctor.patients.update', $patient) }}">
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="gender" class="form-select @error('gender') is-invalid @enderror">
                                ...
                                <option value="male" @selected(old('gender', $patient->user->gender?->value) === 'male')>{{ __('app.male') }}</option>
                                <option value="female" @selected(old('gender', $patient->user->gender?->value) === 'female')>{{ __('app.female') }}</option>
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="national_id" class="form-control @error('national_id') is-invalid @enderror"
                                value="{{ old('national_id', $patient->user->national_id) }}">
                        ...
                        <div class="col-md-6">
                            ...
                            @php $maritalStatus = old('marital_status', $patient->marital_status?->value) @endphp
                            <select name="marital_status" class="form-select @error('marital_status') is-invalid @enderror">
                                <option value="">-- {{ __('app.select') }} --</option>
                                <option value="single" @selected($maritalStatus === 'single')>{{ __('patients.single') }}</option>
                                <option value="married" @selected($maritalStatus === 'married')>{{ __('patients.married') }}</option>
                                <option value="divorced" @selected($maritalStatus === 'divorced')>{{ __('patients.divorced') }}</option>
                                <option value="widowed" @selected($maritalStatus === 'widowed')>{{ __('patients.widowed') }}</option>
                            </select>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-4">
                            ...
                            @php $bloodType = old('blood_type', $patient->blood_type?->value) @endphp
                            <select name="blood_type" class="form-select @error('blood_type') is-invalid @enderror">
                                <option value="">-- {{ __('app.select') }} --</option>
                                @foreach(['A+','A-','B+','B-','AB+','AB-','O+','O-'] as $bt)
                                    <option value="{{ $bt }}" @selected($bloodType === $bt)>{{ $bt }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="current_medications" rows="2"
                                class="form-control @error('current_medications') is-invalid @enderror">{{ old('current_medications', $patient->current_medications) }}</textarea>
                    ...
                    <div class="mt-4 d-flex gap-2">
                        ...
                        <a href="{{ route('doctor.patients.show', $patient) }}" class="btn btn-outline-secondary">{{ __('app.cancel') }}</a>
                    </div>
                </form>
            </div>
        </div>
