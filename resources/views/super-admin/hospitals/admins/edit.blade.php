...
@section('title', __('app.edit') . ' — ' . $user->first_name . ' ' . $user->last_name)
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">{{ __('app.edit') }}: {{ $user->first_name }} {{ $user->last_name }}</h5>
                <a href="{{ route('super-admin.hospitals.admins.index', $hospital) }}" class="btn btn-outline-secondary btn-sm">
                    {{ __('app.back') }}
                </a>
            </div>
            <form method="POST" action="{{ route('super-admin.hospitals.admins.update', [$hospital, $user]) }}">
                @csrf @method('PUT')
                <div class="card-body row g-3">
                    ...
                    <div class="col-md-6">
                        <label class="form-label">{{ __('hospitals.admin_email') }}</label>
                        <input type="email" value="{{ $user->email }}" class="form-control" disabled>
                        <small class="text-muted">{{ __('app.email_not_editable') ?? 'البريد لا يمكن تعديله' }}</small>
                    </div>
                </div>
